...
static void test_repetition_penalties(
    ...
    llama_sampling_softmax_impl(&candidates_p);
    DUMP(&candidates_p);
    llama_sampling_repetition_penalties_impl(&candidates_p, (const llama_token *) last_tokens.data(), last_tokens.size(), repeat_penalty, alpha_frequency, alpha_presence);
    llama_sampling_softmax_impl(&candidates_p);
    DUMP(&candidates_p);
